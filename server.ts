  ...
  app.post("/api/parse-task", async (req, res) => {
    ...
    try {
      ...
      const systemInstruction = `Today is ${now.toISOString()}. 
        ...
        6. RECURRING TASKS: If a user says "every X hours" or "every X hrs", extract the interval as "everyHours" (number). If a user says "twice a day", "three times a day", "3x/day", or similar daily frequency wording, create that many separate task objects with natural timing cues such as morning, midday, afternoon, and evening instead of one vague task.
        ...
        10. MEAL TIMING: Breakfast belongs near the start of the user's awake period, lunch around the middle of that user's awake period, and dinner/supper in the later part of that user's awake period. Do not force meals into conventional 7am/noon/6pm slots unless the user explicitly gives those times. If it is too late in the user's actual day for a meal to make sense, do not reschedule that meal into a strange late slot.
